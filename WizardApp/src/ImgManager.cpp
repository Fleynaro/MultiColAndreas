...
bool ImgFile::Load()
{
	...
	if(!strncmp(header.validator, "VER2", 4))
	{
		...
		for(int i = 0; i < header.entries; ++i)
		{
			...
			if (!strcmp(fExtension, ".dff"))
			{
				if (vehicleDFFnames::getInstance().isVehicleModel(std::string(thisEntry.name))) {
					DFF.emplace_back(thisEntry);
				}
			}
			else if(!strcmp(fExtension, ".col"))
			{

...
BuildManager::BuildManager(std::string &GTADirectory, std::string &OutDirectory, bool SAMPObjects, bool CustomObjects, bool vehicles)
	: GTAInstallDirectory(GTADirectory), OutfileDirectory(OutDirectory), UsingSAMPObjects(SAMPObjects), UsingCustomObjects(CustomObjects), UsingVehicles(vehicles)
{
	InitDirectoryNames();
}
...
bool BuildManager::ExtractImageFiles()
{
	for(std::string &name : ImageFileNames)
	{
		...
		if(thisFile.Load())
		{
			...
			if (this->UsingVehicles)
			{
				for (ImgEntry &e : thisFile.DFF)
				{
					mainWindow->addFormattedMessage("\t\tLoading: %s", e.name);
					...
					dffSection section;
					section.id = 0x0;
					thisFile.handle->seekg(0x0C, std::ios::cur);
					while (thisFile.handle->read((char*)&section, sizeof(dffSection)))
					{
						if (section.id == 0x03) {
							break;
						}
						thisFile.handle->seekg(section.size, std::ios::cur);
					}
					thisFile.handle->seekg(0x0C, std::ios::cur);
					if (section.id != 0x03 || section.size == 0) continue;

					ColFile fileReader(thisFile.handle);
					ColModel modelInfo;
					if (fileReader.Load(&modelInfo))
					{
						modelInfo.modelid = vehicleDFFnames::getInstance().getModelId(modelInfo.name);
						std::transform(modelInfo.name.begin(), modelInfo.name.end(), modelInfo.name.begin(), ::tolower);
						if (modelInfo.name.find("lod") == std::string::npos) COLArray.emplace_back(modelInfo);
					}
					else {
						mainWindow->addFormattedMessage("\t\t\t--- error when a vehicle was loading.");
					}
				}
			}
...
void BuildManager::PrepareDatabaseStructures()
{
	...
	for(auto it = COLArray.begin(); it != COLArray.end(); )
	{
		...
		if((model_it == modelidLookup.end() || model_it->second != it->name) && !vehicleDFFnames::getInstance().isVehicle(it->modelid))
		{
...
bool BuildManager::WriteBinaryFile(const char fname[])
{
	...
	if (ColAndreasBinaryfile.is_open())
	{
		...
		for(ColModel &m : COLArray)
		{
			...
			if (m.modelid == 445) {
				mainWindow->addFormattedMessage("\tm.modelid: %u", m.modelid);
			}
